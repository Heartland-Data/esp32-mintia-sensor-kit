/**
 * @file mlx90640_api.c
 * @brief MLX90640 API implementation for ESP32-IDF
...
/**
 * @brief Dump EEPROM data from MLX90640
 */
...
/**
 * @brief Get frame data from MLX90640
 */
...
/**
 * @brief Extract parameters from EEPROM data
 */
...
/**
 * @brief Set resolution of MLX90640
 */
...
/**
 * @brief Get current resolution of MLX90640
 */
...
/**
 * @brief Set refresh rate of MLX90640
 */
...
/**
 * @brief Get current refresh rate of MLX90640
 */
...
/**
 * @brief Calculate temperature objects
 */
void MLX90640_CalculateTo(uint16_t *frameData, const paramsMLX90640 *params,
  ...
  ktaScale = pow(2, (double)params->KsTa);
  kvScale = pow(2, (double)params->resolutionEE);
...
static void ExtractKsTaParameters(uint16_t *eeData, paramsMLX90640 *mlx90640) {
  float KsTa;
  KsTa = (int8_t)((eeData[60] & 0xFF00) >> 8);
  if (KsTa > 127) {
    KsTa = KsTa - 256;
  }
  KsTa = KsTa / 8192.0f;
...
static void ExtractKtaPixelParameters(uint16_t *eeData,
  ...
  KtaRoCo = (int8_t)((eeData[54] & 0xFF00) >> 8);
  KtaRC[0] = KtaRoCo;

  KtaReCo = (int8_t)(eeData[54] & 0x00FF);
  KtaRC[2] = KtaReCo;

  KtaRoCe = (int8_t)((eeData[55] & 0xFF00) >> 8);
  KtaRC[1] = KtaRoCe;

  KtaReCe = (int8_t)(eeData[55] & 0x00FF);
  KtaRC[3] = KtaReCe;
